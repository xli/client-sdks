...
| GET    | /accounts/:account\_id               | Returns account resource. All executed diem transactions result should be included. |
| POST   | /accounts/:account\_id/payment_uri   | Returns a payment\_uri ([Intent Identifier]) for receiving payment.                 |
| POST   | /accounts/:account\_id/send\_payment | Send a payment to a payee.                                                          |
...
| GET | /kyc_samples | Returns [KycSamples] for clients to use in the test. |


Optional Debugging APIs:

| Method | Path                          | Description                                                                          |
| GET    | /accounts/:account\_id/events | Returns all events happened in the system for the account, ordered by creation time. |
...
| payment_uri | string! | Payment URI[Intent Identifier] for receiving the payment. |
...
| Attribute                  | Type    | Description                                                                                              |
|----------------------------|---------|----------------------------------------------------------------------------------------------------------|
| id                         | string! | Unique identifier of the send payment. Event may reference this id.                                      |
...
| diem\_transaction\_version | uint?!  | Associated Diem transaction version. Set by server if there is diem transaction involved for the action. |
...
[Intent Identifier]: https://dip.diem.com/dip-5/#intent-identifiers
